...
때로는 컨텐츠를 랩핑(wrapping)하기 위한 이상적인 의미론적 요소를 찾기 어려운 상황에 직면할 것입니다. 또, CSS나 JS를 적용하기 위해 멀리 떨어진 요소들을 그룹핑할 필요도 있을 것입니다. 이 경우들을 위해 `<div>`와 `<span>`이 있습니다.

`<span>`은 인라인(inline) 비의미적(non-semantic) 요소로, 컨텐츠를 랩핑할 의미론적 텍스트 요소가 없거나 특정 의미를 부여하고 싶지 않은 경우에만 사용해야 합니다.

`<div>` 역시 블록 레벨 요소라는 것만 제외하면 `<span>`의 설명과 동일합니다.

`<span>`과 `<div>`는 편리하기 때문에 남발하기 쉽습니다. 하지만 이들은 의미론적 가치가 없기 때문에 코드를 복잡하게 만듭니다. 앞서 강조했듯이 적절한 의미론적 요소를 찾을 수 없는 경우에만 최소한으로 사용해야 합니다.