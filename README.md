...
HTML에 대해 학습한 내용을 적습니다. MDN 사이트의 HTML 튜토리얼을 참고했습니다.
...
```html
<p>My cat in very grumpy</p>
```

- 요소(element): `<p>My cat is very grumpy</p>`
- 컨텐츠(content): `My cat is very grumpy`
- 열린 태그(opening tag): `<p>`
- 닫힌 태그(closing tag): `</p>`

어떤 요소는 속성(attribute)을 가질 수 있습니다. 속성은 실제 컨텐츠로 표시되지 않고 요소에 대한 추가적인 정보를 담고 있습니다. '속성명="값"'으로 나타내며 어떤 속성은 값을 가지지 않기도 합니다(ex. `required`).

```html
<p class="cat-describe">My cat in very grumpy</p>
```
...
### 1-3. 블록(Block) 요소 vs 인라인(Inline) 요소
...
### 3-3. 중요(Emphasis)와 강조(Importance)
...
### 5-1. 설명 목록(Description List)

목록의 종류 중에서 앞서 살펴본 순서가 있는 목록(`<ol>`), 순서가 없는 목록(`<ul>`) 외에도 설명 목록(`<dl>`)이 존재합니다.

`<dl>`의 사용 목적은 용어나 정의의 설명, 질문 및 답변처럼 특정 항목에 대해 관련 설명을 표시하기 위함입니다. 구성 요소는 다음과 같습니다.
...
### 5-2. 인용(Quotation)과 출처(Citation)
...
### 5-5. 위/아래 첨자(Superscript / Subscript)
...
`alt`(alternative text) 속성은 이미지에 대한 텍스트 설명으로, 인터넷 연결이 느리거나 렌더링이 오래 걸리는 상황에서 이미지 대신 나타납니다. 다음은 대체 텍스트를 사용해야 하는 이유입니다.

- 사용자가 시각적으로 장애를 가지고 있는 경우 스크린 리더가 그 내용을 읽어줄 수 있습니다.
- 이미지 파일명이나 경로를 잘못 적은 경우를 판별할 수 있습니다.
- 이미지 지원이 되지 않는 브라우저(Lynx)가 존재합니다.
- 검색 엔진은 텍스트를 이용합니다. 검색 엔진이 대체 텍스트와 검색어를 일치시킬 수 있습니다.
- 사용자가 과도한 데이터 전송량을 이유로 이미지 표시를 꺼놓는 경우가 있습니다. 망 사용료가 비싼 국가에서 흔히 볼 수 있는 현상입니다.

`alt` 속성에 무엇을 써야 하는지는 이미지의 존재 이유에 따라 다릅니다. 다음은 그 이유에 대한 예시와 `alt` 사용법입니다.

- 꾸밈(Decoration) : 이미지가 컨텐츠의 일부가 아닌, 단지 꾸밈을 위한 것이라면 CSS를 사용하는 것이 좋습니다. 하지만 HTML을 사용해야 한다면 `alt`에 공백을 적어야(`alt=""`) 스크린 리더가 불필요한 작업을 하지 않습니다.
- 컨텐츠(Content) : 이미지가 중요한 정보를 제공한다면, `alt=""`를 사용하는 대신 메인 텍스트에 정보를 작성하는 것이 좋습니다.
- 링크(Link) : 만약 이미지를 `<a>`에 넣어 링크로 만든다면, `alt`에는 접속이 가능한 링크 텍스트를 제공해야 합니다.
- 텍스트(Text) : 이미지 안에 텍스트를 넣어서는 안됩니다. 만약 그림자 효과(drop shadow)가 필요하다면 CSS를 사용해야 합니다. 하지만 불가피한 경우 `alt`에 관련 텍스트를 제공해야 합니다.

`width`, `height` 속성으로 이미지의 너비와 높이를 지정할 수 있습니다. 값은 정수로 작성하며 픽셀 단위를 의미합니다.

너비와 높이를 따로 지정하는 이유는 HTML과 이미지가 서로 다른 HTTP 요청(request)으로 가져온 별개의 리소스(resource)이기 때문입니다. 다음은 HTML과 이미지 리소스가 사용자에게 보여지는 순서입니다.

1. 브라우저가 HTML을 다운로드 받자마자 사용자에게 페이지를 보여주기 시작합니다.
2. 이미지가 뒤늦게 다운로드 됩니다. 이는 이미지 파일의 용량이 HTML 파일보다 훨씬 크기 때문에 자주 발생하는 일입니다.
3. 이미지 로드가 완료되면 브라우저는 페이지에 이미지를 추가합니다.

이미지는 공간을 차지하기 때문에, HTML만 표시되고 있던 페이지는 이미지가 추가되면서 요소들의 위치가 바뀌게 될 것입니다(ex. 텍스트가 뒤로 밀려남). 만약 사용자가 페이지의 텍스트를 읽고 있었다면, 읽던 위치가 바뀌는 불편함을 겪게 됩니다.

HTML에 이미지의 실제 사이즈를 `width`와 `height` 속성으로 명시했다면, 브라우저는 이미지를 다운로드하기 전에 이미지를 위한 공간이 얼마나 필요한지 알 수 있습니다. 이것은 이미지가 다운로드 되었을 때 브라우저가 주변 컨텐츠를 옮길 필요가 없다는 것을 의미합니다.

주의할 점은, 이미지의 크기를 조정(resize)하는 데에 사용해서는 안됩니다. 이는 이미지 다운로드에 많은 대역폭을 사용하게 되며 이미지 또한 가로, 세로 비율이 깨져 왜곡되어 보일 수 있습니다. 이미지의 크기는 CSS를 사용해 조정해야 합니다.

`<link>`와 마찬가지로, `<img>`에 `title` 속성을 추가해 정보를 제공할 수도 있습니다. 그러나 이는 접근성 문제로 인해 권장되지 않습니다. 주로 스크린 리더의 지원 여부를 알 수 없고, 대부분의 브라우저에서는 마우스를 올려야만 표시되기 때문에 키보드 사용자는 접근할 수 없습니다. 따라서 추가 정보는 `title` 속성보다 본문에 포함시키는 것이 좋습니다.

### 8-2. 이미지 주석(Annotating)

이미지에 캡션(caption)을 추가하는 몇 가지 방법이 있습니다. 다음은 그 예시입니다.
...
위 방법도 가능은 하지만, 이미지와 캡션을 의미적으로 연결하는 것이 없습니다. 이것은 스크린 리더에서 문제가 될 수 있습니다.

`<figure>`와 `<figcaption>`은 이 문제를 해결하기 위해 만들어졌습니다. 피규어(figure)에 의미적 컨테이너를 제공하고 캡션을 연결합니다.
...
접근성의 관점에서 캡션과 `alt` 텍스트는 다른 역할을 합니다. 이미지가 어떠한 이유로 렌더링되지 못했다면 캡션과 `alt`가 모두 나타나게 됩니다. 따라서 두 곳에는 같은 내용이 있으면 안됩니다.

피규어 컨텐츠가 반드시 이미지일 필요는 없습니다. 코드 스니펫(snippet), 오디오, 비디오, 표 등이 될 수 있습니다.

### 8-3. CSS 배경 이미지

페이지에 이미지를 삽입하는 방법으로 HTML이 아닌 CSS를 사용할 수도 있습니다. `background-*` 속성들을 이용합니다. 예를 들어 모든 문단에 배경 이미지를 넣기 위해서는 다음과 같이 작성합니다.
...
CSS를 사용한 임베디드(embedded) 이미지는 HTML 이미지보다 위치 지정 또는 제어가 쉽습니다. 하지만 이러한 이미지는 의미적이지 않고, 대체 텍스트가 없으며, 스크린 리더에서도 보이지 않습니다. 이미지가 컨텐츠 관점에서 의미를 지닌다면 HTML 이미지를 사용하고, 단순히 시각적으로 꾸밀 뿐이라면 CSS 배경 이미지를 사용해야 합니다.